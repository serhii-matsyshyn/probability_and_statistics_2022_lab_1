...
! The provided dataset is split into train and test datasets incorrectly (see information below).
...
### Sample prediction
```{r}
model$predict("reduce a maximum of many jobs")
```
...
To analyze the dataset, we created a histogram with top 10 words and where they are repeated and the number of sentences in each class.
