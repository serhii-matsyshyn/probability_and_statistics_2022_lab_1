...
naiveBayes <- setRefClass("naiveBayes",
    ...
    methods = list(
        ...
        predict = function(message) {
            ...
            prediction_class_0 <- g_class_prob[1] + sum(g_final[g_final$splitted %in% splitted_message & g_final$sentiment == "positive", "prob"])
            prediction_class_1 <- g_class_prob[2] + sum(g_final[g_final$splitted %in% splitted_message & g_final$sentiment == "negative", "prob"])
            prediction_class_2 <- g_class_prob[3] + sum(g_final[g_final$splitted %in% splitted_message & g_final$sentiment == "neutral", "prob"])
            ...
            if (prediction_class_0 > prediction_class_1 && prediction_class_0 > prediction_class_2) {
                return("positive")
            } else if (prediction_class_1 > prediction_class_0 && prediction_class_1 > prediction_class_2) {
                return("negative")
            } else {
                return("neutral")
            }
...
model <- naiveBayes()
model$fit()
model$predict("UPM-Kymmene said its has ` not indicated any interest in any domestic consolidations")
```
